...
function doDijkstraFirstLast(initMinutes: number, width: number, height: number, windsAt: Winds[]): number {
    const dijkstraFirst: Dijkstra<Elfe> = new Dijkstra<Elfe>(
        () => { return { pos: [1, 0], minute: initMinutes } },
    ...
    return dijkstraFirst.shortestPath()?.score!;
}
...
function doPart(input: string): string | number {
    ...
    const resultFirst: number = doDijkstraFirstLast(0, width, height, windsAt);

    const dijkstraLunch: Dijkstra<Elfe> = new Dijkstra<Elfe>(
        () => { return { pos: [width - 2, height - 1], minute: resultFirst } },
        e => e.pos[0] === 1 && e.pos[1] === 0,
        e => {
            const next: Elfe[] = [];
            const [x, y] = e.pos;
            // wait
            ...
            // east
            if (!(x === width - 2 && y === height - 1) && (x + 1 <= width - 2) && isPosFree(windsAt, e.minute + 1, [x + 1, y], width, height)) {
                next.push({ minute: e.minute + 1, pos: [x + 1, y] })
            }
            // west
            if (!(x === width - 2 && y === height - 1) && (x - 1 >= 1) && isPosFree(windsAt, e.minute + 1, [x - 1, y], width, height)) {
                next.push({ minute: e.minute + 1, pos: [x - 1, y] })
            }
            // south
            if ((y + 1 <= height - 2) && isPosFree(windsAt, e.minute + 1, [x, y + 1], width, height)) {
                next.push({ minute: e.minute + 1, pos: [x, y + 1] })
            }
            // north
            if ((x === 1 && y - 1 === 0) || (y - 1 >= 1) && isPosFree(windsAt, e.minute + 1, [x, y - 1], width, height)) {
                next.push({ minute: e.minute + 1, pos: [x, y - 1] })
            }
            return next;
        },
    ...
    const resultLunch = dijkstraLunch.shortestPath()?.score!;

    const resultLast: number = doDijkstraFirstLast(resultFirst + resultLunch, width, height, windsAt);

    return resultFirst + resultLunch + resultLast;
}
